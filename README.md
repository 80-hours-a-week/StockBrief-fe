...
Hosted Amplify 화면에서 추천 후보, 상세 근거, guest 관심종목 화면이 실제로
보이는지 확인할 때는 다음 smoke를 사용한다. 결과에는 HTTP 상태와 화면 요소
확인값만 남기며, raw HTML, provider 원문, localStorage payload는 출력하지
않는다.
